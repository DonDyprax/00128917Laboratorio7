...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,kappa,lambda,gamma,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixH;
    ...
    productRealMatrix(tau*J/(6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    productRealMatrix(J/(6*lambda*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
    ...
    //Preparando matrixH
    gamma = m.getParameter(GAMMA);
    productRealMatrix(gamma*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
